...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 USE RADM2_Model
      ...
      REAL(kind=dp) V(59), F(2), RCT(156)
      REAL(kind=dp) Vdot(59)
...
! Check for the right number of output arguments
     ...
     &Vdot(59)')
      ...
      VPtr = mxGetPr(prhs(1))
      CALL mxCopyPtrToReal8(VPtr,V,59)
      
      FPtr = mxGetPr(prhs(2))
      CALL mxCopyPtrToReal8(FPtr,F,2)
      
      RPtr = mxGetPr(prhs(3))
      CALL mxCopyPtrToReal8(RPtr,RCT,156)

      VdotPtr = mxGetPr(plhs(1))

      CALL Fun( V, F, RCT, Vdot )

      CALL mxCopyReal8ToPtr(Vdot, VdotPtr, 59)
