...
labels = [name if name in label_mechs else "" for name in mech_survey["Short Name"]]
positions = ["middle right" if name in {"RCIM", "CB05", "CBM-Z", "MOZART-4"} 
             ...
             else "middle left" for name in mech_survey["Short Name"]]
...
sizes2 = [25 if name in label_mechs2
...
x_line = np.logspace(np.log10(1), np.log10(100000), 100)
y_line = x_line
fig.add_trace(
    go.Scatter(
        x=x_line,
...
positions2 = [
    ...
    else "middle right" for name in mech_survey_crowded_KI["Short Name"]]
